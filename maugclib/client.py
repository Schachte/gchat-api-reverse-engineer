...
# import cgi  # Removed - deprecated in Python 3.13+
...
class Client:
            ...
            while depth < 10:
                ...
                async with req as resp:
                    ...
                    try:
                        ...
                        params = dict(
                            msg.get_params(header="content-disposition") or []
                        )
    ...
    @staticmethod
    async def read_with_max_size(
        resp: aiohttp.ClientResponse, max_size: int
    ) -> bytearray:
        content_length = int(resp.headers.get("Content-Length", "0"))
        if 0 < max_size < content_length:
            raise exceptions.FileTooLargeError("File size larger than maximum")
        size_str = (
            "unknown length" if content_length == 0 else f"{content_length} bytes"
        )
    ...
    async def upload_file(
        ...
        res = await self._base_request(
            upload_url, None, "", data, headers=headers, method="PUT"
        )
    ...
    async def proto_get_self_user_status(
        ...
        await self._gc_request(
            "get_self_user_status", get_self_user_status_request, response
        )
    ...
    async def proto_mark_group_read_state(
        ...
        await self._gc_request(
            "mark_group_readstate", mark_group_read_state_request, response
        )
        return response
