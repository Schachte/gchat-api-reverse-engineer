...
class Session:
    ...
    def __init__(
        self, cookies: Cookies, proxy: str = None, user_agent: str = None
    ) -> None:
        self._proxy = proxy
        ...
        # Set cookies on their correct domains
        # SID, HSID, SSID belong to .google.com
        # OSID, COMPASS belong to chat.google.com
        google_com_cookies = SimpleCookie()
        chat_cookies = SimpleCookie()

        for key, value in cookies._asdict().items():
            upper_key = key.upper()
            if upper_key in ("SID", "HSID", "SSID"):
                google_com_cookies[upper_key] = value
                google_com_cookies[upper_key].update(
                    {"domain": ".google.com", "path": "/"}
                )
            else:
                chat_cookies[upper_key] = value
                chat_cookies[upper_key].update(
                    {"domain": "chat.google.com", "path": "/"}
                )
        # Update cookies on their respective domains
        google_com_url = URL("https://www.google.com/")
        self._cookie_jar.update_cookies(google_com_cookies, google_com_url)
        self._cookie_jar.update_cookies(chat_cookies, chat_google_com)
    ...
    async def fetch(
        ...
        if res.status != 200:
            logger.info(
                "Request to %s returned unexpected status: %d %s",
                url,
                res.status,
                res.reason,
            )
